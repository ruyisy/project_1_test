...
class SynonymReplacementOptimizer:
    ...
    def _get_state(self, doc, position, history):
        ...
        word_vec = encode([target_word])[0]  
        query_similarity = float(np.dot(word_vec, self.query_vector.T))  
        
        relative_pos = position / len(tokens)
        is_start = float(position == 0)
        is_end = float(position == len(tokens) - 1)
        
        history_data = history.get(position, [])
        num_attempts = len(history_data) / 10  
        avg_reward = np.mean([r for _, r in history_data]) if history_data else 0
    ...
    def optimize_document(self, document, target_score, max_iterations):
        ...
        for iteration in tqdm(range(max_iterations)):
            ...
            score_improvement > improvement_threshold):  
                ...
                stats['final_modifications'][position] = {
                    'original_word': document.split()[position],
                    'final_word': new_word,
        ...
        purtb = len(stats['modified_positions'])/stats['total_words']*100
        
        final_similarity = stats['semantic_similarities'][-1]
        return best_doc, best_score, stats, purtb, final_similarity

    def _compute_reward(self, old_score, new_score, target_score, semantic_similarity, semantic_threshold):
        reward = 0
